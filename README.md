...
Windows 桌面照片管理工具 —— 快速识别并清理重复/相似照片，智能批量重命名，修复照片方向问题，制作标准尺寸证件照，一键替换证件照背景色。支持简体中文、繁体中文、English、日本語四种语言。
...
- 两种模式自由切换：「制作证件照」与「仅更换背景色」
- 上传已有标准证件照，自动识别当前背景颜色（边缘采样算法）
- 选择目标背景色（白色、蓝色、红色），实时预览替换效果
- 基于颜色阈值的智能替换，边缘羽化过渡，人物主体保持不变
- 保留原始 DPI 元数据，支持 JPEG/PNG 输出
- 前后对比预览，确认效果后再保存
...
- **多语言支持**：简体中文、繁体中文（台湾用语习惯）、English、日本語四种语言即时切换
- **拖拽交互**：支持从系统拖拽文件夹/图片导入，去重对比页拖拽调整图片优先级排序
- **主题颜色**：7 种主题颜色（深灰、白色、米色、天蓝、深蓝、克莱因蓝、灰色）
- **文件夹导入**：递归扫描所有子文件夹，支持 JPG、PNG、GIF、BMP、WebP、TIFF
- **设置持久化**：所有偏好自动保存，重启后恢复（含语言偏好）
- **进度反馈**：实时进度条和文件计数
