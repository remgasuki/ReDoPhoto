...
const menuIcons: Record<Feature, JSX.Element> = {
  dedup: (
  ...
  idphoto: (
    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5}
        d="M10 6H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V8a2 2 0 00-2-2h-5m-4 0V5a2 2 0 114 0v1m-4 0h4m-4 4a2 2 0 104 0 2 2 0 00-4 0z" />
    </svg>
  )
}

const menuKeys: Feature[] = ['dedup', 'rename', 'orientation', 'idphoto']
...
export default function Sidebar({ theme, onSettingsClick }: SidebarProps) {
  ...
  const menuLabels: Record<Feature, string> = {
    dedup: t('sidebar.dedup'),
    rename: t('sidebar.rename'),
    orientation: t('sidebar.orientation'),
    idphoto: t('sidebar.idphoto')
  }
      ...
      <div className={`px-4 py-3 border-b ${theme.border}`}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className={`text-[10px] ${theme.textDim}`}>{t('app.title')}</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 py-2 px-2 space-y-1 overflow-y-auto">
        {menuKeys.map((key) => {
          ...
          return (
            ...
            >
              <span className={isActive ? 'text-blue-400' : theme.textDim}>{menuIcons[key]}</span>
              <span className="truncate">{menuLabels[key]}</span>
            </button>
          )
        })}
      </nav>
